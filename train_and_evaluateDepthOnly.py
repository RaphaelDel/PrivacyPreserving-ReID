...
from lightning_class import batch_hard_Transformer_depth_only as Lc
...
def trainModel(train, val, log_folder):
    ...
    train_datasetD = Lc.BatchHardVRAI(img_labels=train, img_dir=DATAFOLDER,
    ...
    val_datasetD = Lc.BatchHardVRAI(img_labels=val, img_dir=DATAFOLDER,
                                    train=True, transformDepth=depthTransform, normaliseDepth = True,
                                    transformRGB=RGBTransform, substract_Depth=True,
                                    maxSequenceLenght=NBFRAMES, nb_images_variations=IMGVARIATIONS)
    ...
    seq_encoder = Lc.SequenceEncoder(EMBEDDING_SIZE, NbFrames=NBFRAMES)

    logger = TensorBoardLogger(log_folder, name='Depth_Transformer_HB') #, version='MeanOverSeq'
    checkpoint_callback = ModelCheckpoint(save_top_k=2, monitor="Val/val_loss")

    trainerD = L.Trainer(accelerator="gpu", max_epochs=MAX_EPOCHS, logger=logger, callbacks=[checkpoint_callback]) # , profiler="simple" for inspecting bottleneck
...
def evaluate(depth_encoder, folder_to_save='./metrics', device='cuda:0'):
    ...
    data_val = rgb_Lc.DepthVRAI(img_labels=val, img_dir=DATAFOLDER, train=False, transformDepth=depthTransform, normaliseDepth = True, transformRGB=RGBTransform, maxSequenceLenght=NBFRAMES)
    ...
    for i in tqdm(range(len(data_val))):
        with torch.no_grad():
            ...
            anchor, _ = depth_encoder(torch.unsqueeze(anchor_depth_sequence, 0).to(device))
            positive, _ = depth_encoder(torch.unsqueeze(postive_depth_sequence, 0).to(device))
